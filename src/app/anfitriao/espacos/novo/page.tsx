...
export default function NovoEspaco() {
  ...
  // Serviços
  const [servicos, setServicos] = useState([{ id: Date.now().toString(), nome: "", preco: "" }]);
  ...
  // ── Funções de serviços ──
  const adicionarServico = () => setServicos([...servicos, { id: Date.now().toString(), nome: "", preco: "" }]);
  const removerServico = (id: string) => setServicos(servicos.filter((s) => s.id !== id));
  const atualizarServico = (id: string, campo: "nome" | "preco", valor: string) =>
  ...
  const removerRegra = (index: number) => setRegras(regras.filter((_, i) => i !== index));
  ...
  const removerFacilidade = (index: number) => setFacilidades(facilidades.filter((_, i) => i !== index));
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Number(data.capacidade) <= 0 || Number(data.area) <= 0 || Number(data.valor) < 0) {
      toast.error("Capacidade, área e valor devem ser números válidos.");
      setLoading(false);
      return;
    }
    ...
    <>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-white py-10">
        <div className="w-full max-w-4xl p-8 rounded-2xl shadow-xl bg-white border border-gray-100">
          <h2 className="text-2xl font-semibold text-gray-800 mb-1">Cadastrar Novo Espaço</h2>
          <p className="text-gray-500 mb-6">Preencha as informações do seu espaço para começar a receber reservas.</p>
           ...
           <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1">
                ...
                <input type="text" name="nome_espaco" placeholder="Ex: Campo do Sorriso" className="w-full border border-gray-300 rounded-xl px-4 py-2.5 mt-1 focus:ring-2 focus:ring-sky-500 outline-none" required />
              </div>
              <div className="flex-1">
                <label className="block text-sm font-medium text-gray-700">Tipo</label>
                <select name="tipo_espaco" className="w-full border border-gray-300 rounded-xl px-4 py-2.5 mt-1 focus:ring-2 focus:ring-sky-500 outline-none" required>
            ...
            <div>
              ...
              <textarea name="descricao" rows={3} placeholder="Fale um pouco sobre o espaço..." className="w-full border border-gray-300 rounded-xl px-4 py-2.5 mt-1 focus:ring-2 focus:ring-sky-500 outline-none resize-none" required></textarea>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1">
                ...
                <input type="number" name="capacidade" min="1" placeholder="Ex: 100" className="w-full border border-gray-300 rounded-xl px-4 py-2.5 mt-1 focus:ring-2 focus:ring-sky-500 outline-none" required />
              </div>
              <div className="flex-1">
                <label className="block text-sm font-medium text-gray-700">Área (m²)</label>
                <input type="number" name="area" min="1" placeholder="Ex: 150" className="w-full border border-gray-300 rounded-xl px-4 py-2.5 mt-1 focus:ring-2 focus:ring-sky-500 outline-none" required />
              </div>
              <div className="flex-1">
                <label className="block text-sm font-medium text-gray-700">Valor (R$)</label>
                <input type="number" name="valor" min="0" step="0.01" placeholder="Ex: 500.00" className="w-full border border-gray-300 rounded-xl px-4 py-2.5 mt-1 focus:ring-2 focus:ring-sky-500 outline-none" required />
              </div>
            </div>
            ...
            <div>
              ...
              <select name="tipo_cobranca" className="w-full border border-gray-300 rounded-xl px-4 py-2.5 mt-1 focus:ring-2 focus:ring-sky-500 outline-none" required>
                <option value="">Selecione</option>
...
<div>
  ...
  <select name="tipo_reserva" className="w-full border border-gray-300 rounded-xl px-4 py-2.5 mt-1 focus:ring-2 focus:ring-sky-500 outline-none" required>
    <option value="">Selecione</option>
    ...
    <option value="manual">Necessita aprovação do anfitrião</option>
            ...
            {/* Upload de fotos */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Fotos do espaço (máx. 5)</label>
              <input type="file" multiple accept="image/*" onChange={handleFotoChange} className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-sky-50 file:text-sky-700 hover:file:bg-sky-100 transition" />
            </div>

            {message && (
              <p className={`text-sm font-medium p-3 rounded-xl ${message.startsWith("✅") ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                {message}
              </p>
            )}

            <button type="submit" disabled={loading} className="w-full bg-sky-500 text-white py-2 rounded-xl font-semibold hover:bg-sky-600 transition">
